    ...
    <div class="bg-gradient-to-r from-purple-700 to-purple-900 text-white py-10 shadow-lg sticky top-0 z-20">
      <div class="max-w-7xl mx-auto px-4">
        <div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
          <div class="flex-1">
            <h1 class="text-2xl sm:text-3xl font-bold leading-tight">CU18 - Bitácora de Operaciones</h1>
...
const buildStreamUrl = (page = bitacora.current_page) => {
  ...
  return `/cu18/auditoria/stream?${params.toString()}`;
};
...
const aplicarFiltros = () => {
  ...
  if (filtros.fecha_desde) params.append('fecha_desde', filtros.fecha_desde);
  if (filtros.fecha_hasta) params.append('fecha_hasta', filtros.fecha_hasta);
  window.location.href = `/cu18/auditoria?${params.toString()}`;
};
...
const limpiarFiltros = () => {
  ...
  filtros.fecha_desde = '';
  filtros.fecha_hasta = '';
  window.location.href = '/cu18/auditoria';
};
...
const formatearFecha = (fechaHora) => {
  return new Date(fechaHora).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' });
};

const formatearHora = (fechaHora) => {
  return new Date(fechaHora).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
};

const getAccionBadge = (accion) => {
  const a = (accion || '').toLowerCase();
  if (a.includes('login') || a.includes('ingreso')) return 'bg-green-100 text-green-800 px-3 py-1 rounded-full text-xs font-semibold';
  if (a.includes('logout') || a.includes('salida')) return 'bg-orange-100 text-orange-800 px-3 py-1 rounded-full text-xs font-semibold';
  if (a.includes('crear') || a.includes('insertar')) return 'bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-xs font-semibold';
  if (a.includes('editar') || a.includes('actualizar')) return 'bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-xs font-semibold';
  if (a.includes('eliminar') || a.includes('borrar')) return 'bg-red-100 text-red-800 px-3 py-1 rounded-full text-xs font-semibold';
